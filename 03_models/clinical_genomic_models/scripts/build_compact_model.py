...
def source_data_tables(predictions: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
...
def write_readme(output_dir: Path, compressed: pd.DataFrame, reference: pd.DataFrame, selected: list[str], config: SplitConfig) -> None:
    ...
    pooled_span = f"{min(config.discovery_years)}-{max(config.validation_years)}"
...
def run(paths: Paths, args: argparse.Namespace) -> None:
    ...
    roc, pr, cal, dca = source_data_tables(predictions)
